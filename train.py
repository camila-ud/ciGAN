...
class CiGAN:
    ...
    def __init__(self, save_name,load_name,patch_size,num_iterations,
                batch_size,new_model,train_vgg,load_vgg,load_weights,l1_factor,save_model):
        self.save_name = save_name
        self.patch_size = patch_size
        self.load_name = load_name
        self.learn_rate = None
           ...
           #history -------
            ...
            while it < int(self.num_iterations):
                ...
                for j in range(1):
                    ...
                    print('========')
                    G_loss_cur = self.train(self.sess, self.generator_solver, self.generator_loss, generator=data_generator, iters=g_iters)
                    print('G loss', G_loss_cur)
                    it += g_iters
                    g_data.append([it,G_loss_cur])
                    print('========')
            ...
            self.plot_loss(np.stack([d_data,g_data]))
            np.savez_compressed('loss{}'.format(self.type),d = d_data,g=g_data)
        tf.reset_default_graph()
    ...
    def plot_loss(self,data):
        ...
        for i in data:
            print(i.shape)
            plt.plot(i[:,0],i[:,1])
        plt.title("Discriminator ang Gen losses ({})".format(self.type))
